export default {
    ...
    HowModelStreamflowDrought: {
        ...
        paragraph2: "To predict streamflow percentiles, we train the model with input data and output data for all of CONUS: <span><ul><li>Input data: <span class='tooltip-group'><span class='tooltip-span'>Static watershed characteristics</span><span id='characteristics-tooltip' class='tooltiptext'>e.g., watershed area, land cover, topography, soil types.</span></span>, antecedent precipitation and streamflow conditions, and upcoming weather forecasts</li><li>Output data: Streamflow percentiles at USGS stream gages. Because streamflow droughts occur when streamflow is unusually low, we specifically train the model to predict low streamflow percentiles by restricting the training dataset to observations below the 30th percentile</li></ul></span>",
