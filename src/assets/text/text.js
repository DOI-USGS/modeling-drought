export default {
    ...
    WhyForecastStreamflowDrought: {
        ...
        paragraph1: "The food we eat, the water we drink, the clothes we buy, and the electricity we use depend on ample amounts of clean, accessible water. When water flowing through rivers and streams drops to unusually low levels—termed “<a href='https://water.usgs.gov/vizlab/what-is-drought/'>streamflow drought</a>”—water managers can have trouble allocating enough water to meet home, farm, business, and energy needs. Streamflow drought forecasts help decision makers anticipate periods of limited water availability so they can develop plans for ensuring the safety and health of their communities. To meet this need, USGS developed AI machine learning (AI/ML) models that predict streamflow drought up to 13 weeks into the future.",
        mapperDiagram: {
    ...
    WhenIsADroughtADrought: {
        ...
        defineLowStreamflow: {
            ...
            paragraph1: "To understand how typical a mean daily streamflow value is, USGS scientists compare it to previous mean daily streamflow values on that same day in earlier years. The chart below shows the observed streamflow values on March 1<sup>st</sup> over 112 years at one USGS <a href='https://labs.waterdata.usgs.gov/visualizations/gages-through-the-ages/index.html'>streamgage</a>.",
            figure1Path: "percentiles_explainer_2.png",
            paragraph2: "Streamflow values are categorized as normal, below normal, or above normal based on how often they occur. These frequency-based bins are called <b>percentiles</b>. For example, the observed streamflow at this site on April 12<sup>th</sup>, 2025, was 1,200 cubic feet per second (cfs). If streamflow at this site on April 12<sup>th</sup> is below 1,200 cfs in 20% of recorded years, then 1,200 cfs is the <b>20<sup>th</sup> percentile streamflow</b> for that site on that date. Another way to say it is, at that site on April 12<sup>th</sup>, streamflow is less than 1,200 cfs 20% of the time. By calculating percentile values for every day of the year, we understand mean daily streamflow values in the context of historical observations.",
            figure2Path: "percentiles_explainer_3.png",
        },
        classifyStreamflowDrought: {
            heading: "Classifying streamflow drought using percentiles",
            paragraph1: "While the USGS streamflow categories shown above are useful for classifying all streamflow observed at a site, the USGS is using the following U.S. Drought Monitor (USDM) categories to classify the low levels of streamflow that indicate streamflow drought:<span><ul><li>Moderate streamflow drought (< 20<sup>th</sup> percentile, USDM D1 drought)</li><li>Severe streamflow drought (< 10<sup>th</sup> percentile, USDM D2 drought)</li><li>Extreme streamflow drought (< 5<sup>th</sup> percentile, USDM D3 drought)</li></ul></span>",
    ...
    HowModelStreamflowDrought: {
        ...
        paragraph1: "To forecast streamflow drought at USGS streamgages across the lower 48 states (the conterminous United States, or CONUS), USGS scientists built an AI/ML model that predicts streamflow percentiles. AI can’t predict streamflow percentiles on its own—scientists must first train the model to learn the relationship between input and output data for thousands of watersheds across the country.",
    ...
    HowWellModelPerform: {
        ...
        forecastTrueFalseRightOrWrong: {
            ...
            questionAll: "<b>Overall</b>, how often is the model <span class='group-label right'>right</span> or <span class='group-label wrong'>wrong</span>?",
            questionNoDrought: "If the model <b>does not predict </b>streamflow <b>drought</b>, how often is it <span class='group-label true-neg'>right</span> or <span class='group-label false-neg'>wrong</span>?",
            questionDrought: "If the model <b>does predict </b>streamflow <b>drought</b>, how often is it <span class='group-label true-pos'>right</span> or <span class='group-label false-pos'>wrong</span>?",
    ...
    HowCaptureUncertainty: {
        ...
        paragraph3: "The model generates these three predicted values by using what are called <b>loss functions</b>.",
        lossFunctionDiagram: {
        ...
        lossFunction: {
            paragraph1: "To generate a <b>median prediction</b>, scientists use a <b>symmetric loss function</b>. This function penalizes under- and overestimations equally. When plotted, the loss function has identical slopes on both sides of zero error—meaning penalties for incorrect predictions increase at the same rate whether the prediction is too high or too low.",
            paragraph2: "To generate a <b>95% quantile prediction </b>(upper bound), scientists use an <b>asymmetric loss function</b> that penalizes underestimations more heavily than overestimations. This directional penalty results in a plot where the left slope (underestimate) is steeper than the right (overestimate).",
            paragraph3: "To generate a <b>5% quantile prediction </b>(lower bound), the <b>asymmetric loss function</b> is reversed. Overestimates are penalized more than underestimates, producing a loss function with a steeper right slope.",
