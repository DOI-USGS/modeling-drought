...
total = tf_d["false_neg"] + tf_d["true_neg"]

ND_ND_list = tf_d["true_neg"] / total * 100.0
ND_D_list = tf_d["false_neg"] / total * 100.0 + ND_ND_list
...
fig_true_false = plt.figure(
    ...
    gid="figure-" + basename_gid_true_false_summary + "-nd",
)
# add axes for the forecast
ax_true_false = fig_true_false.add_axes(
    [0.1, 0.15, 0.85, 0.75], gid="axis-" + basename_gid_true_false_summary + "-nd"
)
...
ax_true_false.set_title(
    "Model outcomes when drought is not predicted",
    loc="left",
    weight="extra bold",
    color="k",
)
...
# make svg
fig_true_false.savefig("Task2/out/fc_tf_sum_nd_desktop.png", dpi=400, metadata=None)
fig_true_false.savefig("Task2/out/fc_tf_sum_nd_desktop.svg", dpi=150, metadata=None)

# remove metadata
remove_metadata_and_fix(
    "Task2/out/fc_tf_sum_nd_desktop.svg",
...
# make svg
fig_true_false.savefig("Task2/out/fc_tf_sum_nd_mobile.svg", metadata=None)

# remove metadata
remove_metadata_and_fix(
    "Task2/out/fc_tf_sum_nd_mobile.svg",
...
# make svg
fig_true_false.savefig("Task2/out/fc_tf_sum_nd_tablet.svg", metadata=None)

# remove metadata
remove_metadata_and_fix(
    "Task2/out/fc_tf_sum_nd_tablet.svg",
