...
y_forecast_lower_raw = forecast_format(forecast_data[0]["lower"] / 100.0)
y_forecast_median_raw = forecast_format(forecast_data[0]["median"] / 100.0)
y_forecast_upper_raw = forecast_format(forecast_data[0]["upper"] / 100.0)
...
for i, forecast_datum in enumerate(forecast_data):
    ...
    f_cloud_lower_list += [
        interpolate.interp1d(
            ...
            forecast_format(forecast_datum["lower"] / 100.0),
        )
    ]
    f_cloud_median_list += [
        interpolate.interp1d(
            dx_forecast_raw_temp.astype(float),
            forecast_format(forecast_datum["median"] / 100.0),
        )
    ]
    f_cloud_upper_list += [
        interpolate.interp1d(
            dx_forecast_raw_temp.astype(float),
            forecast_format(forecast_datum["upper"] / 100.0),
        )
    ]
...
# observation
ax_forecast.scatter(
    x_forecast_raw, y_training_raw, color="tab:red", alpha=1.0, gid="observation"
)
...
x_cloud_smooth = np.linspace(0.0, 91.0, 92)
# ax_forecast.plot(x_cloud , y_cloud_lower, color="tab:blue", linestyle="--",alpha=1.0,gid='forecast_lower_'+str(j))
ax_forecast.plot(
    x_forecast[j] + x_cloud_smooth.astype(int),
    PchipInterpolator(np.array(offset) * 7, np.array(y_cloud_lower))(x_cloud_smooth),
    color="tab:blue",
    linestyle="--",
    alpha=1.0,
    gid="forecast_lower_" + str(j),
)
# ax_forecast.plot(x_cloud , y_cloud_median, color='k',linestyle="-",alpha=1.0,gid='forecast_middl_'+str(j))
ax_forecast.plot(
    x_forecast[j] + x_cloud_smooth.astype(int),
    PchipInterpolator(np.array(offset) * 7, np.array(y_cloud_median))(x_cloud_smooth),
    color="k",
    linestyle="-",
    alpha=1.0,
...
ax_forecast.plot(
    x_forecast[j] + x_cloud_smooth.astype(int),
    PchipInterpolator(np.array(offset) * 7, np.array(y_cloud_upper))(x_cloud_smooth),
    color="tab:orange",
    linestyle="--",
    alpha=1.0,
    gid="forecast_upper_" + str(j),
)
...
ax_forecast.plot(
    x_forecast, y_training, color="tab:red", alpha=0.0, gid="observation-full-forecast"
)
...
fig.savefig("Task2/out/forecast_cloud.svg", metadata=None)

# remove metadata
remove_metadata_and_fix(
    "Task2/out/forecast_cloud.svg", "src/assets/svgs/forecast_cloud.svg"
)
