...
bbox1 = selectable_text(
    ax_forecast,
    labels_bump,
    lower_interp(np.datetime64(date_range[-1]).astype(float)) / 100.0,
    "5% Quantile",
    "w",
    np.array(lower_color_limit) / 256.0,
    "k",
    "top",
    "left",
    "LOWER-TAG",
)
bbox2 = selectable_text(
    ax_forecast,
    labels_bump,
...
bbox3 = selectable_text(
    ax_forecast,
    labels_bump,
    upper_interp(np.datetime64(date_range[-1]).astype(float)) / 100.0,
    "95% Quantile",
    "k",
    np.array(upper_color_limit) / 256.0,
    "k",
    "bottom",
    "left",
    "UPPER-TAG",
)
bbox4 = selectable_text(
    ax_forecast,
    0.99,
    1.02,
    "Toggle Observations",
...
ax_forecast.annotate(
    ...
    va="center",
    ha="center",
    xy=(0.5 * (bbox1.x0 + bbox1.x1), bbox1.y1),
    xytext=annotation_label_loc_lf,
    xycoords="axes fraction",
    arrowprops=dict(
        facecolor="black",
        gid="annotation_buttons1_arrow1",
        shrinkA=8,
        shrinkB=10,
        arrowstyle="->",
        connectionstyle="arc3,rad=-0.3",
...
ax_forecast.annotate(
    ...
    va="center",
    ha="center",
    xy=(bbox2.x0, 0.5 * (bbox2.y0 + bbox2.y1)),
    xytext=annotation_label_loc_lf,
    xycoords="axes fraction",
    arrowprops=dict(
        facecolor="black",
        gid="annotation_buttons1_arrow2",
        shrinkA=12,
        shrinkB=10,
        arrowstyle="->",
        connectionstyle="arc3,rad=-0.3",
...
ax_forecast.annotate(
    ...
    va="center",
    ha="center",
    xy=(bbox3.x0, 0.5 * (bbox3.y0 + bbox3.y1)),
    xytext=annotation_label_loc_lf,
    xycoords="axes fraction",
    arrowprops=dict(
        facecolor="black",
        gid="annotation_buttons1_arrow3",
        shrinkA=10,
        shrinkB=10,
        arrowstyle="->",
        connectionstyle="arc3,rad=-0.3",
...
ax_forecast.annotate(
    ...
    va="center",
    ha="center",
    xy=(0.5 * (bbox4.x0 + bbox4.x1), bbox4.y0),
    xytext=annotation_label_loc_lf,
    xycoords="axes fraction",
    arrowprops=dict(
        facecolor="black",
        gid="annotation_buttons1_arrow4",
        shrinkA=8,
        shrinkB=20,
        arrowstyle="->",
        connectionstyle="arc3,rad=-0.3",
